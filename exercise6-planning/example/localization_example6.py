...
class Exercise(object):
    ...
    def gyrocallback(self, gyro):
        if (0):
            ...
            head_x_new = (self.head_x * math.cos(gyro.gyro.z) -
                          self.head_y * math.sin(gyro.gyro.z))
            head_y_new = (self.head_x * math.sin(gyro.gyro.z) +
                          self.head_y * math.cos(gyro.gyro.z))
    ...
    def slope_fx(self, x, datas):
        ...
        delta1 = np.array([d, 0])
        delta2 = np.array([0, d])
        J1 = (self.loss_fun(x + delta1, datas) -
              self.loss_fun(x - delta1, datas)) / (2.0 * d)
        J2 = (self.loss_fun(x + delta2, datas) -
              self.loss_fun(x - delta2, datas)) / (2.0 * d)
        return [J1, J2]
